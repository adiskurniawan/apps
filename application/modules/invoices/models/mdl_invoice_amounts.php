...
class Mdl_Invoice_Amounts extends CI_Model {
	...
	public function calculate($invoice_id)
	{
		...
		FROM invoice_item_amounts WHERE item_id IN (SELECT item_id FROM invoice_items WHERE invoice_id = " . $this->db->escape($invoice_id) . ")");

		$invoice_amounts = $query->row();

		// Get the amount already paid
		$query = $this->db->query("SELECT SUM(payment_amount) AS invoice_paid FROM payments WHERE invoice_id = " . $this->db->escape($invoice_id));
	...
	public function calculate_invoice_taxes($invoice_id)
	{
		...
		if ($invoice_tax_rates)
		{
			...
			$this->db->query("UPDATE invoice_amounts SET invoice_tax_total = (SELECT SUM(invoice_tax_rate_amount) FROM invoice_tax_rates WHERE invoice_id = " . $this->db->escape($invoice_id) . ") WHERE invoice_id = " . $this->db->escape($invoice_id));
		...
		switch ($period)
		{
			...
			case 'year':
				return $this->db->query("
					...
					FROM invoice_amounts 
					WHERE invoice_id IN 
					(SELECT invoice_id FROM invoices WHERE YEAR(invoice_date_created) = YEAR(NOW()))")->row()->total_invoiced;
			case 'last_year':
				return $this->db->query("
					SELECT SUM(invoice_total) AS total_invoiced 
					FROM invoice_amounts 
					WHERE invoice_id IN 
					(SELECT invoice_id FROM invoices WHERE YEAR(invoice_date_created) = YEAR(NOW() - INTERVAL 1 YEAR))")->row()->total_invoiced;
			default:
				return $this->db->query("SELECT SUM(invoice_total) AS total_invoiced FROM invoice_amounts")->row()->total_invoiced;
		}
	}
		...
		switch ($period)
		{
			...
			case 'year':
				return $this->db->query("SELECT SUM(invoice_paid) AS total_paid 
					FROM invoice_amounts 
					WHERE invoice_id IN 
					(SELECT invoice_id FROM invoices WHERE YEAR(invoice_date_created) = YEAR(NOW()))")->row()->total_paid;
			case 'last_year':
				return $this->db->query("SELECT SUM(invoice_paid) AS total_paid 
					FROM invoice_amounts 
					WHERE invoice_id IN 
					(SELECT invoice_id FROM invoices WHERE YEAR(invoice_date_created) = YEAR(NOW() - INTERVAL 1 YEAR))")->row()->total_paid;
			default:
				return $this->db->query("SELECT SUM(invoice_paid) AS total_paid FROM invoice_amounts")->row()->total_paid;
		}
	}
		...
		switch ($period)
		{
			...
			case 'year':
				return $this->db->query("SELECT SUM(invoice_balance) AS total_balance 
					FROM invoice_amounts 
					WHERE invoice_id IN 
					(SELECT invoice_id FROM invoices WHERE YEAR(invoice_date_created) = YEAR(NOW()))")->row()->total_balance;
			case 'last_year':
				return $this->db->query("SELECT SUM(invoice_balance) AS total_balance 
					FROM invoice_amounts 
					WHERE invoice_id IN 
					(SELECT invoice_id FROM invoices WHERE YEAR(invoice_date_created) = (YEAR(NOW() - INTERVAL 1 YEAR)))")->row()->total_balance;
			default:
				return $this->db->query("SELECT SUM(invoice_balance) AS total_balance FROM invoice_amounts")->row()->total_balance;
		}
	}
