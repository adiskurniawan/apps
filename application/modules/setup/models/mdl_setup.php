...
class Mdl_Setup extends CI_Model {
    ...
    public function upgrade_tables()
    {
        ...
        foreach ($sql_files as $sql_file)
        {
            if (substr($sql_file, -4) == '.sql')
            {
                ...
                // $update_applied = $this->db->get('versions')->row()->update_applied;
    ...
    {
        $this->db->insert('invoice_groups', array('invoice_group_name'    => 'Invoice Default', 'invoice_group_next_id' => 1));
        $this->db->insert('invoice_groups', array('invoice_group_name'    => 'Quote Default', 'invoice_group_prefix'  => 'QUO', 'invoice_group_next_id' => 1));
    }
    ...
    private function install_default_settings()
    {
        ...
        $default_settings = array(
            'default_language'          => $this->session->userdata('lang'),
            'date_format'               => 'm/d/Y',
            'currency_symbol'           => '$',
    ...
    public function upgrade_013_1_1_3()
    {
        ...
        $invoices = $this->db->select('invoice_id')->get('invoices')->result();
        ...
        $this->db->query("ALTER TABLE `invoices` ADD UNIQUE (`invoice_url_key`)");
    }
