...
class Invoices extends Guest_Controller {
    ...
    public function status($status = 'open', $page = 0)
    {
        ...
        switch ($status)
        {
            case 'open':
                ...
                break;
            case 'closed':
                $this->mdl_invoices->is_closed()->where_in('invoices.client_id', $this->user_clients);
                break;
            case 'overdue':
                $this->mdl_invoices->is_overdue()->where_in('invoices.client_id', $this->user_clients);
                break;
        }
    ...
    public function view($invoice_id)
    {
        ...
        $invoice = $this->mdl_invoices->where('invoices.invoice_id', $invoice_id)->where_in('invoices.client_id', $this->user_clients)->get()->row();
