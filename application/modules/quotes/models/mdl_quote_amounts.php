...
class Mdl_Quote_Amounts extends CI_Model {
    ...
    public function calculate($quote_id)
    {
		...
		FROM quote_item_amounts WHERE item_id IN (SELECT item_id FROM quote_items WHERE quote_id = " . $this->db->escape($quote_id) . ")");
    ...
    public function calculate_quote_taxes($quote_id)
    {
        ...
        if ($quote_tax_rates)
        {
            ...
            $this->db->query("UPDATE quote_amounts SET quote_tax_total = (SELECT SUM(quote_tax_rate_amount) FROM quote_tax_rates WHERE quote_id = " . $this->db->escape($quote_id) . ") WHERE quote_id = " . $this->db->escape($quote_id));
        ...
        switch ($period)
        {
            ...
            case 'year':
                return $this->db->query("
					...
					FROM quote_amounts 
					WHERE quote_id IN 
					(SELECT quote_id FROM quotes WHERE YEAR(quote_date_created) = YEAR(NOW()))")->row()->total_quoted;
            case 'last_year':
                return $this->db->query("
					SELECT SUM(quote_total) AS total_quoted 
					FROM quote_amounts 
					WHERE quote_id IN 
					(SELECT quote_id FROM quotes WHERE YEAR(quote_date_created) = YEAR(NOW() - INTERVAL 1 YEAR))")->row()->total_quoted;
            default:
                return $this->db->query("SELECT SUM(quote_total) AS total_quoted FROM quote_amounts")->row()->total_quoted;
        }
    }
