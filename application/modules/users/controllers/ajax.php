...
class Ajax extends Admin_Controller {
    ...
    public function save_user_client()
    {
        ...
        if ($client->num_rows() == 1)
        {
            ...
            if ($user_id)
            {
                ...
                $user_client = $this->mdl_user_clients->where('user_clients.user_id', $user_id)->where('user_clients.client_id', $client_id)->get();
    ...
    public function load_user_client_table()
    {
        if ($session_user_clients = $this->session->userdata('user_clients'))
        {
            ...
            $data = array(
                'id'           => NULL,
                'user_clients' => $this->mdl_clients->where_in('clients.client_id', $session_user_clients)->get()->result()
            );
        }
        else
        {
            ...
            $data = array(
                ...
                'user_clients' => $this->mdl_user_clients->where('user_clients.user_id', $this->input->post('user_id'))->get()->result()
            );
        }
