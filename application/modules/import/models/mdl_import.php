...
class Mdl_Import extends Response_Model {
    ...
    public $primary_key      = 'imports.import_id';
    ...
    public function default_select()
    {
        $this->db->select("SQL_CALC_FOUND_ROWS imports.*, 
            (SELECT COUNT(*) FROM import_details WHERE import_table_name = 'clients' AND import_details.import_id = imports.import_id) AS num_clients,
            (SELECT COUNT(*) FROM import_details WHERE import_table_name = 'invoices' AND import_details.import_id = imports.import_id) AS num_invoices,
            (SELECT COUNT(*) FROM import_details WHERE import_table_name = 'invoice_items' AND import_details.import_id = imports.import_id) AS num_invoice_items,
            (SELECT COUNT(*) FROM import_details WHERE import_table_name = 'payments' AND import_details.import_id = imports.import_id) AS num_payments", FALSE);
    }
                ...
                foreach ($headers as $key => $header)
                {
                    ...
                    elseif ($header == 'item_tax_rate')
                    {
                        ...
                        if ($data[$key] > 0)
                        {
                            ...
                            {
                                $this->db->insert('tax_rates', array('tax_rate_name'    => $data[$key], 'tax_rate_percent' => $data[$key]));
                                $data[$key] = $this->db->insert_id();
                            }
                        }
                        else
                ...
                foreach ($headers as $key => $header)
                {
                    ...
                    elseif ($header == 'payment_method')
                    {
                        ...
                        if ($data[$key])
                        {
                            ...
                            {
                                $this->db->insert('payment_methods', array('payment_method_name' => $data[$key]));
                                $data[$key] = $this->db->insert_id();
                            }
                        }
                        else
    ...
    public function delete($import_id)
    {
        ...
        $import_details = $this->db->where('import_id', $import_id)->get('import_details')->result();
